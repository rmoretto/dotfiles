...
if [ ${IS_LAPTOP} -eq 0 ]; then
  ...
  [ ! -z ${SECONDARY_MONITOR} ] && MONITOR=${SECONDARY_MONITOR} polybar -c ~/.config/polybar/config.ini --reload secondary &
else
  ...
  [ ! -z ${SECONDARY_MONITOR} ] && MONITOR=${SECONDARY_MONITOR} polybar -c ~/.config/polybar/config.ini --reload secondary_notebook &
fi
